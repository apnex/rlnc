...
const piece = new CodedPiece(1, Buffer.alloc(64, 1), Buffer.alloc(1024, 0xFF));
const buf = PacketSerializer.serialize(piece, config.PROTOCOL);
const out = PacketSerializer.deserialize(buf, config.PROTOCOL);

if(out.genId === 1 && out.data.length === 1024) console.log("[PASS] Serialization OK");
else console.error("[FAIL] Serialization mismatch");
